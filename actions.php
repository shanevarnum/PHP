<?php
    ...
    include("functions.php");
        ...
        if ($_POST['loginActive'] == "0") {
            ...
            if (mysqli_num_rows($result) > 0) $error = "The chosen email address has already been taken.";
            else {
            ...
            $row = mysqli_fetch_assoc($result);
                ...
                if ($row['password'] == md5(md5($row['id']).$_POST['password'])) {
                    ...
                    $error = "This user name and password combination was not found. Please try again.";
		...
		if ($_GET['action'] == 'postVent') {
			
			if (!$_POST['ventContent']) {
						
				echo "Your post is empty!";
						
				} else if (strlen($_POST['ventContent']) > 250) {
				
				echo "Your post exceeded the character limit!";
				
			} else {
				
				mysqli_query($link, "INSERT INTO vents (`vent`, `userid`, `datetime`) VALUES ('". mysqli_real_escape_string($link, $_POST['ventContent'])."', ". mysqli_real_escape_string($link, $_SESSION['id']).", NOW())");
